# rbsSeeker
A new and unified software with Poisson and Hypergeometric modeling, to identify significant RBP-binding events from various kinds of CLIP-seq data, such as HITS-CLIP, PAR-CLIP, iCLIP and eCLIP data. rbsSeeker is also suitable for miCLIP data analysis.
...
- [output](#output)
  - [peaks](#output-peaks)
  - [mutation sites](#output-mutation)
  - [truncation sites](#output-truncation)
  - [insertion sites](#output-truncation)
- [usage](#usage)
...
# input
* A genome sequences in fasta format is required by `--fa` argument, which chromsome's name should be the same with input `bam` file.
