...
double readPEBamToBed12Map(BamReader &reader, chromBed12Map &bed12Hash)
// must at same chromosome
{
  int i            = 0;
  double totalNum  = 0;
  ...
  {
    if (bam1.IsMapped() == true && bam1.RefID == bam1.MateRefID) // same chrom
    {
      // get paired bam items
      reader.GetNextAlignment(bam2);
      if (bam2.IsMapped() == true)
      {
        ...
        else
        {
          ...
          CBed12 *bed12Ptr = twoBed6ToBed12(bed1, bed2);
          freeBed6Item(bed1);
          freeBed6Item(bed2);
